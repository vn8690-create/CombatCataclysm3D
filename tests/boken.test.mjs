...
assert.equal(boken.resolvedNodes.has('tokyo_gate'), true, 'resolved story nodes should be tracked');
...
assert.equal(boken.resolveCurrentNode().ok, true);
...
assert.equal(boken.resolvedNodes.has('konbini_detour'), true);
...
assert.equal(boken.resolvedNodes.has('printer_brawl'), false);
result = boken.resolveCurrentNode({ win: true });
assert.equal(result.ok, true);
assert.equal(boken.resolvedNodes.has('printer_brawl'), true);
...
assert.equal(restored.resolvedNodes.has('printer_brawl'), true, 'resolved node state should survive serialization');
